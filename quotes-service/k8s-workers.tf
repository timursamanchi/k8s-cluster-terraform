...
resource "aws_launch_template" "workers_lt" {
  ...
  image_id      = var.custom_ami_id # AMI ID for the worker node instances (should match Kubernetes compatibility requirements)
  instance_type = "t3.micro" # Instance type; t3.micro is for testing/light workloads. 
                             # For production workloads, consider t3.medium or larger based on resource needs (CPU, memory).
